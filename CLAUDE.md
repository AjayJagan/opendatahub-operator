...
**DSCInitialization** (`api/dscinitialization/v1/`)
...
- **Creates:**
  ...
  - Service CRs (Monitoring, Auth, Gateway)
  - Default HardwareProfiles
  - Network policies and service mesh configuration
- **Must be created before DataScienceCluster**

**DataScienceCluster** (`api/datasciencecluster/v1/`)
...
- `ServiceMesh`: Service mesh configuration and integration
...
User creates DSCInitialization
  └─> DSCI Controller reconciles:
      ...
      ├─> Creates service mesh resources
...
- `api/datasciencecluster/v1,v2/` - DataScienceCluster CRD
- `api/dscinitialization/v1,v2/` - DSCInitialization CRD
- `api/services/v1alpha1/` - Service CRDs (Monitoring, Auth, Gateway, ServiceMesh)
- `api/features/v1/` - FeatureTracker for cross-namespace resources
- `api/infrastructure/v1/` - HardwareProfile, Certificate, Serverless, ServiceMesh
...
- `pkg/feature/` - Feature API for cross-namespace resource management
...
### Feature API
- Located in `pkg/feature/`
- Purpose: Manage resources across namespaces with proper ownership
- Creates `FeatureTracker` CRs as cluster-scoped owners
- Use case: Deploy monitoring resources to multiple namespaces

### Resource Ownership
- Component CRs owned by DataScienceCluster
- Deployed resources owned by component CRs (via FeatureTracker for cross-namespace)
...
- Storage version: `v1` (in `api/*/v1/`)
- Served version: `v2` (in `api/*/v2/`)
- Conversion webhooks handle version translation
