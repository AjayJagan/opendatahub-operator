    ...
    for (const [key, value] of Object.entries(process.env)) {
        if (key.startsWith(specPrefix)) {
            ...
            for (const [manifestComponentName, componentInfo] of manifestComponents) {
                const envFormat = manifestComponentName.toLowerCase().replace(/[\/\-]/g, '_');

                // workbenches/kf-notebook-controller -> kf_notebook_controller
                const envFormatWithoutPrefix = manifestComponentName.toLowerCase()
                    .replace(/^workbenches[\/\-]/, '')
                    .replace(/[\/\-]/g, '_');

                if (envFormat === componentKey.toLowerCase() ||
                    envFormatWithoutPrefix === componentKey.toLowerCase()) {
