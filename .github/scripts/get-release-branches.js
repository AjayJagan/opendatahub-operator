    ...
    github.request('GET /repos/{owner}/{repo}/issues/{issue_number}/comments', {
        ...
        result.data.forEach((issue) => {
            ...
            if (issueCommentBody.includes("#Release#")) {
                ...
                components = components.slice(releaseIdx + 1, components.length)
                ...
                components.forEach(component => {
                    if (regex.test(component)) {
                        ...
                        if (componentName.trim() === "notebook-controller") {
                            core.exportVariable("component_spec_odh-notebook-controller".toLowerCase(), branchName.trim());
                            core.exportVariable("component_spec_kf-notebook-controller".toLowerCase(),branchName.trim());
                        } else {
                            core.exportVariable("component_spec_" + componentName.toLowerCase(), branchName.trim());
                        }
                    }
                })
            }
